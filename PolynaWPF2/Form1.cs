...
namespace PolynaWPF2
{
    public partial class Form1 : Form
    {
        ...
        {
            SoundPlayer soundPlayer = new SoundPlayer(@"C:\Windows\Media\Alarm03.wav");
            soundPlayer.Play();
            for (int i = 0; i < n * n; i++)
            {
                for (int j = 0; j < n * n; j++)
        ...
        private void button2_Click_1(object sender, EventArgs e)
        {
            SoundPlayer soundPlayer = new SoundPlayer(@"C:\Windows\Media\Alarm03.wav");
            soundPlayer.Play();
            MessageBox.Show("Игровое поле представляет собой квадрат размером 9×9, разделённый на меньшие квадраты со стороной в 3 клетки. Цель игры - заполнить пустые клетки цифрами от 1 до 9 так, чтобы в каждой строке, в каждом столбце и в каждом малом квадрате 3×3 каждая цифра встречалась только один раз.");
            return;
        }

        private void timer1_Tick_1(object sender, EventArgs e)
        {
            label1.Text = DateTime.Now.ToString("h:mm:ss.fff");
        }
